...
Of note is the 'host'. The default 'host' for queries is set to
'useastdb.ensembl.org' which only supports the current or current minus 1
version of VEP. For older versions the 'host' would have to be specified.

<<hostChange, eval=FALSE>>=
param <- VEPFlags(flags=list(host="ensembldb.ensembl.org"))
@
