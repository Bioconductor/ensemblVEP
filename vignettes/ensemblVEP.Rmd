...
Prior to Bioconductor 3.19, the ensemblVEP package worked
through an interface between Perl and MySQL.

In 3.19 this package focuses on use of the VEP component
...
library(rjsoncons)
names(fromJSON(jmespath(jans, "[*]")))
```

Annotation of the most severe consequence known will typically
be of interest:
```{r doj2}
table(fromJSON(jmespath(jans, "[*].most_severe_consequence")))
```
